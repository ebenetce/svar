...
| `conjugatebvarm(m,p,...)` | Matrix-Normal-IW (`Sigma kron V`) | analytic `conjugatebvarm` | `Mu,V,Omega,DoF` |
| `semiconjugatebvarm(m,p,...)` | independent Normal-IW | Gibbs -> `empiricalbvarm` | `Mu,V,Omega,DoF` |
| `normalbvarm(m,p,...)` | normal coeffs, Sigma fixed | analytic `normalbvarm` | `Mu,V,Sigma` |
...
The built-in four and `weakbvarm` / `uniformirbvarm` are **data-free**: you
pass `Y` at `estimate`. `minnesotabvarm` takes `Y` **up front** and stores it
(see section 6).
...
`Sigma kron V`, so own- and cross-variable tightness cannot differ (that is why
`mniw` has no free `lambda2`). If you need asymmetric own-vs-cross shrinkage,
use the `"inw"` (semiconjugate) or `"normal"` family, which carry a free
`lambda2`.
...
| `"normal"` | `litterman`, `fixed`, `fixedsigma` | normal coeffs, fixed Sigma | analytic | `lambda2` |
...
|---|---|---|---|
| `lambda1` | overall Minnesota tightness (paper lambda) | all | `0.2` |
| `lambda2` | cross-variable relative tightness | `inw`, `normal` only | `1` (pinned in `mniw`) |
| `lambda3` | lag-decay exponent; variance proportional to `1 / lag^(2*lambda3)` | all | `1` |
| `lambda4` | sum-of-coefficients tightness (paper mu) | `mniw` only | `Inf` (off) |
| `lambda5` | dummy-initial-observation tightness (paper delta) | `mniw` only | `Inf` (off) |
...
- Litterman starting point: `PriorMean=1`, `lambda1~=0.2`, `lambda3~=1`, `Vc`
  large (`1e4`-`1e6`). Turn on `lambda4`/`lambda5` (e.g. `1`) for persistent
  macro data.
...
- Rather than guess, tune by marginal likelihood - see section 5.

---

## 5. Tuning by marginal likelihood (`glp` + `hyperprior`)

`glp` maximises the analytic marginal likelihood (plus any hyperpriors) over
the free hyperparameters, for the conjugate `mniw` family only. Each
hyperparameter is in one of three states, so the free/fixed split is implicit
in what you pass:
...
`hyperprior(dist, mode, sd, Bounds=...)` takes an interpretable `(mode, sd)`
pair for `"Gamma"`, `"InverseGamma"`, or `"Beta"`. Some reference densities have
no mean or variance and cannot be reached from `(mode, sd)` - e.g. the
...
1-by-`m` vector (fixed), a 2-by-`m` `[lower; upper]` matrix (free, flat), a
scalar `hyperprior` (broadcast), or a 1-by-`m` `hyperprior` array.

**Integrating over the hyperparameters.** Ask for a third output plus
`NumDraws` to run GLP's Metropolis step instead of only maximising:
...
`chain` holds per-draw hyperparameters (`chain.lambda1`, `chain.Psi`, ...) and
the VAR parameters they imply (`chain.Coefficients` `m*k x m x NumDraws`,
`chain.Sigma` `m x m x NumDraws`), plus `chain.AcceptanceRate`. Tune
`ProposalScale` for about 0.2-0.3 acceptance (`2.38/sqrt(d)` for `d` free
hyperparameters is a good start). Set `rng` before the call. The chain draws
feed stage 5 directly. `info.Hessian` / `info.ProposalCovariance` expose the
proposal geometry.
