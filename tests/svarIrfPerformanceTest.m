classdef svarIrfPerformanceTest < matlab.perftest.TestCase
    %svarIrfPerformanceTest Benchmarks for impulse response calculations.
    ...
    methods (TestMethodSetup)
        function setupProblem(testCase, Problem)
            [testCase.Mdl, testCase.Impact] = ...
                svarIrfPerformanceTest.deterministicVarModel( ...
                Problem.NumSeries, Problem.NumLags);
            testCase.Horizon = Problem.Horizon;
            [testCase.ArLagOp, testCase.MaLagOp] = ...
                svarIrfPerformanceTest.armaLagOperators(testCase.Mdl);
        end
    end

    methods (Test, TestTags = {'Performance'})
        function testSvarIrf(testCase)
            mdl = testCase.Mdl;
            impact = testCase.Impact;
            horizon = testCase.Horizon;
            while testCase.keepMeasuring
                responses = svar.irf(mdl, impact, horizon);
            end
            testCase.verifySize(responses, [horizon + 1 mdl.NumSeries mdl.NumSeries]);
        end

        function testArmaIrfOrthogonalized(testCase)
            ...
            while testCase.keepMeasuring
                responses = armairf(arLagOp, maLagOp, InnovCov=covariance, ...
                    NumObs=horizon + 1, Method="orthogonalized");
            end
            testCase.verifySize(responses, [horizon + 1 numSeries numSeries]);
        end

        function testArmaIrfGeneralized(testCase)
            arLagOp = testCase.ArLagOp;
            maLagOp = testCase.MaLagOp;
            covariance = testCase.Mdl.Covariance;
            horizon = testCase.Horizon;
            ...
            while testCase.keepMeasuring
                responses = armairf(arLagOp, maLagOp, InnovCov=covariance, ...
                    NumObs=horizon + 1, Method="generalized");
            end
            testCase.verifySize(responses, [horizon + 1 numSeries numSeries]);
        end

        function testVarmIrfOrthogonalized(testCase)
            mdl = testCase.Mdl;
            horizon = testCase.Horizon;
            while testCase.keepMeasuring
                responses = irf(mdl, NumObs=horizon + 1, ...
                    Method="orthogonalized");
            end
            testCase.verifySize(responses, [horizon + 1 mdl.NumSeries mdl.NumSeries]);
        end

        function testVarmIrfGeneralized(testCase)
            mdl = testCase.Mdl;
            horizon = testCase.Horizon;
            while testCase.keepMeasuring
                responses = irf(mdl, NumObs=horizon + 1, ...
